...
def load_static_mnist(args, **kwargs):
    ...
    with open("DATASETS/MNIST_static/binarized_mnist_train.amat") as f:
        lines = f.readlines()
    x_train = lines_to_np_array(lines).astype("float32")
    with open("DATASETS/MNIST_static/binarized_mnist_valid.amat") as f:
        lines = f.readlines()
    x_val = lines_to_np_array(lines).astype("float32")
    with open("DATASETS/MNIST_static/binarized_mnist_test.amat") as f:
        lines = f.readlines()
...
def load_caltech101silhouettes(args, **kwargs):
    ...
    caltech_raw = loadmat(
        os.path.join(
            "../data", "Caltech101Silhouettes", "caltech101_silhouettes_28_split1.mat"
        )
    )
...
if __name__ == "__main__":
    ...
    plot = lambda p, x: torchvision.utils.save_image(
        x.view(x.size(0), 1, args.input_size[1], args.input_size[2]),
        p,
        normalize=True,
    ...
    for x in tr:
        ...
        xt = (
            torch.tensor(qt.fit_transform(x.view(x.size(0), -1).numpy()))
            .float()
            .view(x.size())
        )
        # print(x)
        ...
        for buckets in [2, 4, 8, 16]:
            ...
            quintiles = np.percentile(
                x.numpy(), 100 * np.linspace(0, 1, buckets + 1)[:-1]
            )
            print(quintiles)
