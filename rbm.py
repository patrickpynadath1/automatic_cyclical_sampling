...
class BernoulliRBM(BaseModel):
    def __init__(self, n_visible, n_hidden, data_mean=None):
        ...
        if data_mean is not None:
            init_val = (data_mean / (1.0 - data_mean)).log()
            self.b_v.data = init_val
            self.init_dist = dists.Bernoulli(probs=data_mean)
        else:
            self.init_dist = dists.Bernoulli(probs=torch.ones((n_visible,)) * 0.5)
    ...
    def logp_v_unnorm_beta(self, v, beta):
        if len(beta.size()) > 0:
            beta = beta[:, None]
        vW = v @ self.W.t() * beta
        sp = torch.nn.Softplus()(vW + self.b_h[None]).sum(-1) - torch.nn.Softplus()(
            self.b_h[None]
        ).sum(-1)
        # vt = (v * self.b_v[None]).sum(-1)
...
class LatticeIsingModel(nn.Module):
    def __init__(
        ...
        super().__init__()
        g = ig.Graph.Lattice(
            dim=[dim] * lattice_dim, circular=True
        )  # Boundary conditions
        ...
        self.bias = nn.Parameter(
            torch.ones((dim**lattice_dim,)).float() * init_bias,
...
class ERIsingModel(nn.Module):
    def __init__(
        self, n_node, avg_degree=2, init_bias=0.0, learn_G=False, learn_bias=False
    ):
        super().__init__()
...
class LatticePottsModel(nn.Module):
    def __init__(
        ...
        self.bias = nn.Parameter(
            torch.ones((dim**2, n_out)).float() * init_bias, requires_grad=learn_bias
        )
    ...
    def mix(self):
        off_diag = (
            -(torch.ones((self.n_out, self.n_out)) - torch.eye(self.n_out)).to(self.G)
            * self.sigma
        )
...
class DensePottsModel(nn.Module):
    def __init__(self, dim, n_out=20, init_bias=0.0, learn_J=False, learn_bias=False):
        super().__init__()
        self.J = nn.Parameter(
            torch.randn((dim, dim, n_out, n_out)) * 0.01, requires_grad=learn_J
        )
        self.bias = nn.Parameter(
            torch.ones((dim, n_out)).float() * init_bias, requires_grad=learn_bias
        )
        self.dim = dim
        self.n_out = n_out
